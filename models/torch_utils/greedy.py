...
class GreedyCellB(nn.Module):
    def __init__(self, config):
        super(GreedyCellB, self).__init__()
...
#         self.tag_size = config['tag_size']
#         self.random_seed = config['random_seed']
#         self.true_label_prob = config['true_label_prob']
#         self.act_fn = config.get('hidden_act', 'relu')
#         assert self.act_fn in ['relu', 'gelu']
#         self.num_pre_preds = config['num_pre_preds']
#         self.num_pre_tokens = config['num_pre_tokens']
...
#         self.hidden_size = config['hidden_size']
#         self.pred_embedding_dim = config['pred_embedding_dim']
#         self.fc_feats_dropout_prob = config['fc_feats_dropout_prob']
#         self.fc_mix_feats_dropout_prob = config['fc_mix_feats_dropout_prob']
#         self.fc_pre_preds_dropout_prob = config['fc_pre_preds_dropout_prob']
#
#         self.rand = random.Random(self.random_seed)
#
#         self.layer_pred_emb = nn.Embedding(self.tag_size + self.num_pre_preds, self.pred_embedding_dim)
#         self.feats_pad_bng = nn.Parameter(torch.empty(1, 1, self.hidden_size))
#         self.feats_pad_end = nn.Parameter(torch.empty(1, 1, self.hidden_size))
#
#         self.fc_feats = nn.Linear(self.in_hidden_size, self.hidden_size)
#         self.fc_feats_dropout = nn.Dropout(self.fc_feats_dropout_prob)
#
#         self.fc_mix_feats = nn.Sequential(
#             nn.Linear((self.num_pre_tokens + self.num_nxt_tokens + 1) * self.hidden_size, self.hidden_size),
#             nn.ReLU(True) if self.act_fn == 'relu' else Gelu()
#         )
#         self.fc_mix_feats_dropout = nn.Dropout(self.fc_mix_feats_dropout_prob)
#
#         self.fc_pre_preds = nn.Sequential(
#             nn.Linear(self.num_pre_preds * self.pred_embedding_dim, self.hidden_size),
#             nn.ReLU(True) if self.act_fn == 'relu' else Gelu()
#         )
#         self.fc_pre_preds_dropout = nn.Dropout(self.fc_pre_preds_dropout_prob)
#
#         self.layer_out = nn.Sequential(
#             nn.Linear(self.hidden_size * 2, self.hidden_size),
#             nn.ReLU(True) if self.act_fn == 'relu' else Gelu(),
#             nn.Linear(self.hidden_size, self.tag_size)
#         )
#
#         self.reset_parameters()
#
...
#         batch_size, seq_length, _ = feats.shape
#
#         bng_tags = torch.range(start=-self.num_pre_preds, end=-1, step=1, dtype=torch.long, device=device).unsqueeze(0).repeat((batch_size, 1))
#
#         # [batch_size, seq_length, pred_embedding_dim]
#         true_embs = self.tag_id_mod(torch.cat((bng_tags, labelss), dim=1))
#         true_embs = self.layer_pred_emb(true_embs)
...
#         pred_embs = self.layer_pred_emb(pred_embs)
#
#         # [batch_size, num_pre_tokens + seq_length + num_nxt_tokens, hidden_size]
#         feats = self.padding_feats(self.fc_feats_dropout(self.fc_feats(feats)), lengths)
#
#         probs = None
#         for seq in range(seq_length):
#             feat_seq = seq + self.num_pre_tokens
#             pred_seq = seq + self.num_pre_preds
#
#             cat_feats = feats[:, feat_seq - self.num_pre_tokens: feat_seq + self.num_nxt_tokens + 1, :].reshape((batch_size, -1))
#             cur_feat = self.fc_mix_feats_dropout(self.fc_mix_feats(cat_feats))
#
#             if self.rand.random() < self.true_label_prob:
#                 cat_preds = true_embs[:, pred_seq - self.num_pre_preds: pred_seq, :].reshape((batch_size, -1))
#             else:
#                 cat_preds = pred_embs[:, pred_seq - self.num_pre_preds: pred_seq, :].reshape((batch_size, -1))
#             pre_pred = self.fc_pre_preds_dropout(self.fc_pre_preds(cat_preds))
#
#             cur_prob = self.layer_out(torch.cat((cur_feat, pre_pred), dim=-1))
#
#             cur_pred = torch.argmax(cur_prob, dim=-1)
#             cur_pred_emb = self.layer_pred_emb(cur_pred)
#             pred_embs = torch.cat((pred_embs, cur_pred_emb.unsqueeze(1)), dim=1)
#
#             probs = cur_prob.unsqueeze(1) if probs is None else torch.cat((probs, cur_prob.unsqueeze(1)), dim=1)
#
#         loss = F.nll_loss(torch.log(torch.softmax(probs, dim=-1).clamp(min=1e-9).transpose(1, 2)), labelss, ignore_index=ignore_index)
#         return loss
#
#     def predict(self, feats, lengths):
#         device = feats.device
#         batch_size, seq_length, _ = feats.shape
#
#         # [batch_size, seq_length, pred_embedding_dim]
#         pred_embs = torch.range(start=-self.num_pre_preds, end=-1, step=1, dtype=torch.long, device=device).unsqueeze(0).repeat((batch_size, 1))
#         pred_embs = self.tag_id_mod(pred_embs)
#         pred_embs = self.layer_pred_emb(pred_embs)
#
#         # [batch_size, num_pre_tokens + seq_length + num_nxt_tokens, hidden_size]
#         feats = self.padding_feats(self.fc_feats(feats), lengths)
#
#         probs = None
#         for seq in range(seq_length):
#             feat_seq = seq + self.num_pre_tokens
#             pred_seq = seq + self.num_pre_preds
#
#             cur_feat = self.fc_mix_feats(
#                 feats[:, feat_seq - self.num_pre_tokens: feat_seq + self.num_nxt_tokens + 1, :].reshape((batch_size, -1))
#             )
#
#             pre_pred = self.fc_pre_preds(
#                 pred_embs[:, pred_seq - self.num_pre_preds: pred_seq, :].reshape((batch_size, -1))
#             )
#
#             cur_prob = self.layer_out(torch.cat((cur_feat, pre_pred), dim=-1))
#
